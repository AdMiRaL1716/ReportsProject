            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    <div class="col-lg-6 col-7">
                        <h6 class="h2 text-white d-inline-block mb-0">{{ __('tests.change') }}</h6>
                        <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                            <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                                <li class="breadcrumb-item"><a href="{{route('home')}}"><i class="fas fa-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="{{route('tests')}}">{{ __('tests.test') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ __('tests.change') }}</li>
                            </ol>
                        </nav>
                    </div>
                    ...
                    <form method="POST">
                        ...
                        <div class="card-header">
                            <div class="row align-items-center">
                                <div class="col-8">
                                    <h3 class="mb-0">{{ __('tests.change') }}</h3>
                                </div>
                                ...
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="id_element" class="form-control @error('id_element') is-invalid @enderror" required>
                                                @foreach($elements as $element)
                                                    @if($element->id == $test->id_element)
                                                        <option value="{{$element->id}}">{{$element->name}}, {{$element->symbol}} - {{ __('tests.choice') }}</option>
                                                    @endif
                                                @endforeach
                                                @foreach($elements as $element)
                                                    @if($element->id != $test->id_element)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="id_method" class="form-control @error('id_method') is-invalid @enderror" required>
                                                @foreach($methods as $method)
                                                    @if($method->id == $test->id_method)
                                                        <option value="{{$method->id}}">{{$method->name}} - {{ __('tests.choice') }}</option>
                                                    @endif
                                                @endforeach
                                                @foreach($methods as $method)
                                                        @if($method->id != $test->id_method)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="id_unit" class="form-control @error('id_unit') is-invalid @enderror" required>
                                                @foreach($units as $unit)
                                                    @if($unit->id == $test->id_unit)
                                                        <option value="{{$unit->id}}">{{$unit->name}} - {{ __('tests.choice') }}</option>
                                                    @endif
                                                @endforeach
                                                @foreach($units as $unit)
                                                        @if($unit->id != $test->id_unit)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="id_customer" class="form-control @error('id_customer') is-invalid @enderror" required>
                                                @foreach($customers as $customer)
                                                    @if($customer->id == $test->id_customer)
                                                        <option value="{{$customer->id}}">{{$customer->firstname}} {{$customer->lastname}} - {{ __('tests.choice') }}</option>
                                                    @endif
                                                @endforeach
                                                @foreach($customers as $customer)
                                                        @if($customer->id != $test->id_customer)
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="result_one" class="form-control @error('result_one') is-invalid @enderror" value="{{ $test->result_one }}" required placeholder="{{ __('tests.result') }} 1*"/>
                                            @error('result_one')
                                            <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="result_two" class="form-control @error('result_two') is-invalid @enderror" value="{{ $test->result_two }}" required placeholder="{{ __('tests.result') }} 2*"/>
                                            @error('result_two')
                                            <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="result_three" class="form-control @error('result_three') is-invalid @enderror" value="{{ $test->result_three }}" required placeholder="{{ __('tests.result') }} 3*"/>
                                            @error('result_three')
                                            <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
